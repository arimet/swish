...
export function ConfirmDialog({ open, title, message, confirmLabel, cancelLabel, danger, saisieAttendue, onConfirm, onClose }: {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && fermer()}>
      <DialogContent className="sm:max-w-sm border-none bg-[var(--c-card)] p-5 text-[var(--c-text)]">
        ...
        <div className="mt-3 flex gap-2">
          <button onClick={fermer} className="flex-1 rounded-xl bg-[var(--c-card2)] py-2.5 text-sm font-bold transition hover:bg-[var(--c-border)]">{libelleAnnuler}</button>
          <button
