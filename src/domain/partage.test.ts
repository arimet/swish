...
describe('encoder / decoder', () => {
  ...
  it('refuse une charge à moitié valide, jusqu’au fond des tableaux', async () => {
    ...
    const bon = await transportDe(complet())
    ...
    expect(await decoder(await abime((t) => { delete t.temps[0].pions[0].at }))).toBeNull()
    // Une position qui n'est pas un nombre fini : `NaN` traverse `typeof`.
    expect(await decoder(await abime((t) => { t.temps[0].pions[0].at.x = null }))).toBeNull()
    // Un objet posé sans position.
    expect(await decoder(await abime((t) => { delete t.objets[0].at }))).toBeNull()
    // Un ballon qui n'est ni porté ni posé.
    expect(await decoder(await abime((t) => { t.temps[0].ballon = {} }))).toBeNull()
    // Une flèche sans tracé exploitable.
    expect(await decoder(await abime((t) => { t.temps[0].fleches[0].points = [{ x: 0.1, y: 0.1 }] }))).toBeNull()
    // Un poste hors de l'effectif.
    expect(await decoder(await abime((t) => { t.temps[0].pions[0].poste = 9 }))).toBeNull()
...
async function transportDe(s: Schema): Promise<Record<string, unknown>> {
  const recu = (await decoder(await encoder(s)))!
