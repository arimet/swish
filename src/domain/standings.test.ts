...
const our = (id: string, opponentId: string, pa: number, pb: number, date = '2026-01-10', league = 'Poule A'): Match => {
...
describe('standings', () => {
  it('applies the scale: a win is 2 points, a loss 1', () => {
    const [table] = standings([our('m1', 'b', 10, 5)], [], TEAMS)
    expect(table.league).toBe('Poule A')
    expect(table.lines.map((l) => [l.id, l.wins, l.losses, l.pts])).toEqual([['a', 1, 0, 2], ['b', 0, 1, 1]])
  })

  it('combines our games and the entered results', () => {
    const table = standings([our('m1', 'b', 10, 5)], [saisi('r1', 'c', 'd', 70, 60)], TEAMS)[0]
  ...
  it('ignores an entered result that duplicates one of our games', () => {
    ...
    const table = standings([our('m1', 'b', 10, 5)], [saisi('r1', 'a', 'b', 99, 1)], TEAMS)[0]
    const us = table.lines.find((l) => l.id === 'a')!
    expect(us.played).toBe(1)
    expect(us.pointsFor).toBe(20) // our real score, not the 99 copied in
  })

  it('ignores the duplicate whichever way round home and away are', () => {
    const table = standings([our('m1', 'b', 10, 5)], [saisi('r1', 'b', 'a', 1, 99)], TEAMS)[0]
    expect(table.lines.find((l) => l.id === 'a')!.played).toBe(1)
  })

  it('separates the leagues', () => {
    const tables = standings([our('m1', 'b', 10, 5)], [saisi('r1', 'c', 'd', 70, 60, '2026-01-10', 'Coupe')], TEAMS)
