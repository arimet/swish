 ...
 */
export function fusionnerMatchs(stocke: Match, entrant: Match): Match {
  ...
  const retires = [...new Set([...(stocke.retires ?? []), ...(entrant.retires ?? [])])]
  const raye = new Set(retires)

  const parId = new Map<string, GameEvent>()
  for (const e of [...stocke.events, ...entrant.events]) if (!raye.has(e.id)) parId.set(e.id, e)
  ...
  const events = [...parId.values()].sort((a, b) => a.wallClock - b.wallClock || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
  ...
  return {
    ...
    ...entrant,
    events,
    ...(retires.length ? { retires } : {}),
    status: plusAvance(stocke.status, entrant.status),
  }
}

const RANG = { setup: 0, live: 1, finished: 2 } as const
 ...
 */
export function plusAvance(a: Match['status'], b: Match['status']): Match['status'] {
