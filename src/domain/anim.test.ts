...
describe('instantane', () => {
  it('rend exactement le temps de départ à part 0, et le suivant à part 1', () => {
    const s = deuxTemps()
    expect(instantane(s, { temps: 0, part: 0 }).markers).toEqual(s.temps[0].markers)
    expect(instantane(s, { temps: 0, part: 1 }).markers).toEqual(s.temps[1].markers)
  })

  it('interpole en ligne droite le pion sans flèche', () => {
    const s = deuxTemps()
    const p = instantane(s, { temps: 0, part: 0.5 }).markers.find((q) => q.position === 1)!
  ...
  it('laisse strictement immobile un pion qui ne bouge pas', () => {
    ...
    const p = instantane(s, { temps: 0, part: 0.37 }).markers.find((q) => q.position === 3)!
  ...
  it('recale la flèche : un tracé dessiné à côté mène quand même aux positions réelles', () => {
    ...
    const tout = (part: number) => instantane(s, { temps: 0, part }).markers.find((q) => q.position === 1)!.at
  ...
  it('n’expose aucune flèche : l’animation montre les joueurs, pas les traits', () => {
    ...
    expect(instantane(s, { temps: 0, part: 0.5 }).arrows).toEqual([])
  })
  ...
  it('mène le ballon au nouveau porteur quand il change de mains', () => {
    ...
    const depart = s.temps[0].markers.find((q) => q.position === 1)!.at
    const arrivee = s.temps[1].markers.find((q) => q.position === 3)!.at
  ...
  it('fait suivre au ballon la flèche de passe, pas la corde', () => {
    ...
    const b = instantane(s, { temps: 0, part: 0.5 }).ball as { x: number; y: number }
    expect(b.y).toBeGreaterThan(0.7)             // haut de la cloche, loin de la corde
  })

  it('garde le ballon porté quand le porteur ne change pas', () => {
    const s = deuxTemps()
    expect(instantane(s, { temps: 0, part: 0.5 }).ball).toEqual({ side: 'offense', position: 1 })
  })

  it('compte les transitions, et rend le dernier temps au-delà', () => {
    const s = deuxTemps()
    expect(transitions(s)).toBe(1)
    expect(instantane(s, { temps: 5, part: 0.5 }).markers).toEqual(s.temps[1].markers)
  })
})
...
describe('instantane — les lignes de déplacement', () => {
  const ligneDe = (s: Play, part: number, position: number) =>
    instantane(s, { temps: 0, part }, true).arrows.find((f) => f.from.position === position)
  ...
  it('trace la passe quand le ballon change de mains', () => {
    const s = deuxTemps()
    s.temps[1] = { ...s.temps[1], ball: { side: 'offense', position: 3 } }
    const pass = instantane(s, { temps: 0, part: 0.5 }, true).arrows.find((f) => f.stroke === 'pass')!
  ...
  it('ne trace pas de passe quand le ballon ne change pas de mains', () => {
    const s = deuxTemps()
    expect(instantane(s, { temps: 0, part: 0.5 }, true).arrows.some((f) => f.stroke === 'pass')).toBe(false)
  })

  it('n’émet rien sur le dernier temps, qui n’a pas de suite', () => {
    const s = deuxTemps()
    expect(instantane(s, { temps: 1, part: 0 }, true).arrows).toEqual([])
  })
