...
describe('snapshot', () => {
  it('rend exactement le temps de départ à part 0, et le suivant à part 1', () => {
    const s = deuxTemps()
    expect(snapshot(s, { steps: 0, part: 0 }).markers).toEqual(s.steps[0].markers)
    expect(snapshot(s, { steps: 0, part: 1 }).markers).toEqual(s.steps[1].markers)
  })

  it('interpole en ligne droite le pion sans flèche', () => {
    const s = deuxTemps()
    const p = snapshot(s, { steps: 0, part: 0.5 }).markers.find((q) => q.position === 1)!
  ...
  it('laisse strictement immobile un pion qui ne bouge pas', () => {
    ...
    const p = snapshot(s, { steps: 0, part: 0.37 }).markers.find((q) => q.position === 3)!
  ...
  it('recale la flèche : un tracé dessiné à côté mène quand même aux positions réelles', () => {
    ...
    const tout = (part: number) => snapshot(s, { steps: 0, part }).markers.find((q) => q.position === 1)!.at
...
describe('snapshot — les lines de déplacement', () => {
  const ligneDe = (s: Play, part: number, position: number) =>
    snapshot(s, { steps: 0, part }, true).arrows.find((f) => f.from.position === position)
  ...
  it('trace la passe quand le ballon change de mains', () => {
    const s = deuxTemps()
    s.steps[1] = { ...s.steps[1], ball: { side: 'offense', position: 3 } }
    const pass = snapshot(s, { steps: 0, part: 0.5 }, true).arrows.find((f) => f.stroke === 'pass')!
  ...
  it('ne trace pas de passe quand le ballon ne change pas de mains', () => {
    const s = deuxTemps()
    expect(snapshot(s, { steps: 0, part: 0.5 }, true).arrows.some((f) => f.stroke === 'pass')).toBe(false)
  })

  it('n’émet rien sur le dernier temps, qui n’a pas de suite', () => {
    const s = deuxTemps()
    expect(snapshot(s, { steps: 1, part: 0 }, true).arrows).toEqual([])
  })
